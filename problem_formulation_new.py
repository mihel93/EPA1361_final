...
def get_model_for_problem_formulation(problem_formulation_id):
    ...
    # 8-objectives PF:
        ...
        outcomes.append(ScalarOutcome('OverIjsel Dike Investment Costs', variable_name=['{}_Dike Investment Costs'.format(dike)
                                         for dike in function.dikelist[len(function.dikelist)-1:len(function.dikelist)]], function=sum_over,
                                      kind=direction))
                           
        outcomes.append(ScalarOutcome('OverIjsel Expected Annual Damage', variable_name=['{}_Expected Annual Damage'.format(dike)
                                         for dike in function.dikelist[len(function.dikelist)-1:len(function.dikelist)]], function=sum_over,
                                      kind=direction))
